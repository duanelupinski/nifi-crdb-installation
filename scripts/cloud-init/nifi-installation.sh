...
echo "generating certificates for the nifi cluster if they don't already exist..."
rm -rf ~/tls
for host in ${nodes[@]}; do
	nodestr=""
	clientdn=""
	...
	remote="${installer}@${hostip}"
	if [ ! -d ~/tls ]; then
		keystorePasswd=$(openssl rand -base64 36)
		keyPasswd=${keystorePasswd}
		trustStorePassword=$(openssl rand -base64 36)
		command="$(cat <<-EOF
			if [ ! -d ~/tls ]; then
				sudo \${NIFI_TOOLKIT_HOME}bin/tls-toolkit.sh standalone \
					--hostnames "${nodestr::-1}" \
					--certificateAuthorityHostname "${hostip}" \

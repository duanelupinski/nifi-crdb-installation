...
while getopts "h?i:dr:p:k:u:w:l:z:N:I:U:W:P:F:D:C:O:M:L:S" opt; do
  case "${opt}" in
	h|\?) help=true;;
	i) installer=$OPTARG;;
...
if [ $help = true ]; then
	...
	echo "node:        for each node in the cluster provide the hostname and ip address of the node as node-id:hostname=ip.address, the id must be a number starting at 1, i.e."
	echo "                 node-1:nifi-pkg-1=192.168.86.27 node-2:nifi-pkg-2=192.168.86.35 node-3:nifi-pkg-3=192.168.86.38"
...
## else get, report and check the remaining arguments
	...
	if [[ -n "${registry_host}" ]]; then
		...
		put "node-id" $registry_host 0
		put "node" $registry_host $registry_ip
	fi
	...
	if [[ ${#nodes[@]} -lt ${min_nodes} ]]; then
		echo "ERROR: at least 3 server nodes are required for a nifi cluster"
		valid=false
	fi
	if [ $valid = false ]; then
		exit 1
...
echo "stopping existing nifi and other related managed services..."
for host in ${nodes[@]}; do
	...
	if [[ ${host} != ${registry_host} ]]; then
		...
		command="$(cat <<-EOF
			sudo systemctl stop nifi-registry
		EOF
		)"
	fi
...
echo "enabling user for nifi installation..."
for host in ${nodes[@]}; do
	command="$(cat <<-EOF
		## enable the user for nifi
		which passwd &> /dev/null || sudo apt-get install -y passwd
...
for host in ${nodes[@]}; do
	if [[ ${host} != ${registry_host} ]]; then
		...
		command="$(cat <<-EOF
			...
			## download and extract the nifi registry package
			if [ ! -d "${installation_folder}nifi-registry-${nifi_version}/" ]; then
				echo "installing nifi registry binaries for version ${nifi_version}..."
				if [ ! -f "${installation_folder}nifi-registry-${nifi_version}-bin.zip" ]; then
					sudo wget -P ${installation_folder} ${dist}/nifi/${nifi_version}/nifi-registry-${nifi_version}-bin.zip
					sudo chown ${username}: ${installation_folder}nifi-registry-${nifi_version}-bin.zip
				fi
				sudo unzip ${installation_folder}/nifi-registry-${nifi_version}-bin.zip -d ${installation_folder}
				sudo chown -R ${username}: ${installation_folder}nifi-registry-${nifi_version}/
			fi

			## set nifi registry home and escape the forward slashes in the path before storing in the environment
			nifi_registry_home="${installation_folder}nifi-registry-${nifi_version}/"
			nifi_registry_home_esc=\$(eval echo \${nifi_registry_home} | sed 's/\//\\\\\//g')
			\$(grep -q ^NIFI_REGISTRY_HOME= /etc/environment)
			if [ \$? -eq 0 ]; then
				## if the environment points to another nifi registry home then replace it with the latest
				echo "updating nifi registry home in the environment..."
				sudo sed -i "s/^NIFI_REGISTRY_HOME=.*/NIFI_REGISTRY_HOME=\${nifi_registry_home_esc}/g" /etc/environment
			else
				## otherwise add nifi registry home to the environment
				echo "adding nifi registry home to the environment..."
				## make sure we have a new line in our file
				file=/etc/environment
				if [[ -s \${file} && -z "\$(tail -c 1 \${file})" ]]; then
					echo "new line at end of \${file}!"
				...
				echo NIFI_REGISTRY_HOME=\${nifi_registry_home} | sudo tee -a /etc/environment > /dev/null
			fi

			## and reload the nifi registry home environment variables
			. /etc/environment
			echo "NIFI_REGISTRY_HOME=\${NIFI_REGISTRY_HOME}"
		EOF
		)"
	fi
	remote="${installer}@$(get node ${host})"
...
for host in ${nodes[@]}; do
	if [[ ${host} != ${registry_host} ]]; then
		...
		echo "updating http properties for the nifi registry..."
		command="$(cat <<-EOF
			if [ ${secure} = true ]; then
				sudo sed -i "s/^nifi.registry.web.http.host=.*/nifi.registry.web.http.host=/g" \${NIFI_REGISTRY_HOME}conf/nifi-registry.properties
				sudo sed -i "s/^nifi.registry.web.http.port=.*/nifi.registry.web.http.port=/g" \${NIFI_REGISTRY_HOME}conf/nifi-registry.properties
				sudo sed -i "s/^nifi.registry.web.https.host=.*/nifi.registry.web.https.host=${registry_ip}/g" \${NIFI_REGISTRY_HOME}conf/nifi-registry.properties
				sudo sed -i "s/^nifi.registry.web.https.port=.*/nifi.registry.web.https.port=19443/g" \${NIFI_REGISTRY_HOME}conf/nifi-registry.properties
			else
				sudo sed -i "s/^nifi.registry.web.http.host=.*/nifi.registry.web.http.host=${registry_ip}/g" \${NIFI_REGISTRY_HOME}conf/nifi-registry.properties
				sudo sed -i "s/^nifi.registry.web.http.port=.*/nifi.registry.web.http.port=18080/g" \${NIFI_REGISTRY_HOME}conf/nifi-registry.properties
				sudo sed -i "s/^nifi.registry.web.https.host=.*/nifi.registry.web.https.host=/g" \${NIFI_REGISTRY_HOME}conf/nifi-registry.properties
				sudo sed -i "s/^nifi.registry.web.https.port=.*/nifi.registry.web.https.port=/g" \${NIFI_REGISTRY_HOME}conf/nifi-registry.properties
			fi
		EOF
		)"
	fi
...
if [ ${secure} = true ]; then
	...
	for host in ${nodes[@]}; do
		...
		for node in ${nodes[@]}; do
			...
			nodeid=$((${nodeid} + 2))
			xmlcmd+=" -d /authorizers/userGroupProvider/property[@name='\"Initial User Identity ${nodeid}\"'] -s /authorizers/userGroupProvider -t elem -n propertyTMP -v \"CN=$(get node ${node}), OU=NIFI\" -i //propertyTMP -t attr -n name -v \"Initial User Identity ${nodeid}\" -r //propertyTMP -v property"
		done
		if [[ ${host} != ${registry_host} ]]; then
		...
		for node in ${nodes[@]}; do
			nodeid=$(get node-id ${node})
			xmlcmd+=" -d /authorizers/accessPolicyProvider/property[@name='\"Node Identity ${nodeid}\"'] -s /authorizers/accessPolicyProvider -t elem -n propertyTMP -v \"CN=$(get node ${node}), OU=NIFI\" -i //propertyTMP -t attr -n name -v \"Node Identity ${nodeid}\" -r //propertyTMP -v property"
		done
		if [[ ${host} != ${registry_host} ]]; then
			xmlcmd+=" \${NIFI_HOME}conf/authorizers.xml"
		else
...
for host in ${nodes[@]}; do
	if [[ ${host} != ${registry_host} ]]; then
		...
		command="$(cat <<-EOF
			which xmlstarlet &> /dev/null || sudo apt-get install -y xmlstarlet
			sudo xmlstarlet ed -P -L -u /configuration/appender[@name='"APP_FILE"']/rollingPolicy/maxHistory -v 720 \${NIFI_REGISTRY_HOME}conf/logback.xml
			sudo xmlstarlet ed -P -L -u /configuration/appender[@name='"APP_FILE"']/rollingPolicy/totalSizeCap -v "256MB" \${NIFI_REGISTRY_HOME}conf/logback.xml
			sudo xmlstarlet ed -P -L -u /configuration/appender[@name='"BOOTSTRAP_FILE"']/rollingPolicy/maxHistory -v 30 \${NIFI_REGISTRY_HOME}conf/logback.xml
			sudo xmlstarlet ed -P -L -a /configuration/appender[@name='"BOOTSTRAP_FILE"']/rollingPolicy/maxHistory -t elem -n totalSizeCap -v "128MB" \${NIFI_REGISTRY_HOME}conf/logback.xml
			sudo xmlstarlet ed -P -L -u /configuration/appender[@name='"EVENTS_FILE"']/rollingPolicy/maxHistory -v 30 \${NIFI_REGISTRY_HOME}conf/logback.xml
			sudo xmlstarlet ed -P -L -a /configuration/appender[@name='"EVENTS_FILE"']/rollingPolicy/maxHistory -t elem -n totalSizeCap -v "128MB" \${NIFI_REGISTRY_HOME}conf/logback.xml
		EOF
		)"
	fi
	remote="${installer}@$(get node ${host})"
...
for host in ${nodes[@]}; do
	if [[ ${host} != ${registry_host} ]]; then
		...
		command="$(cat <<-EOF
			nifi_home_esc=\$(eval echo \${NIFI_REGISTRY_HOME} | sed 's/\//\\\\\//g')
			sudo sed -i "s/=\.\//=\${nifi_home_esc}/g" \${NIFI_REGISTRY_HOME}conf/nifi-registry.properties
			sudo sed -i "s/\.\//\${nifi_home_esc}/g" \${NIFI_REGISTRY_HOME}conf/authorizers.xml
			sudo sed -i "s/=\.\//=\${nifi_home_esc}/g" \${NIFI_REGISTRY_HOME}conf/bootstrap.conf
			sudo sed -i "s/\.\//\${nifi_home_esc}/g" \${NIFI_REGISTRY_HOME}conf/providers.xml
		EOF
		)"
	fi
	remote="${installer}@$(get node ${host})"
...
for host in ${nodes[@]}; do
	...
	if [[ ${host} != ${registry_host} ]]; then
		...
		command="$(cat <<-EOF
			if [ -f "/etc/systemd/system/nifi-registry.service" ]; then
				sudo rm /etc/systemd/system/nifi-registry.service
			fi
			echo "[Unit]" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "Description=Apache NiFi Registry" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "[Service]" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "Type=forking" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "User=${username}" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "Group=${username}" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "WorkingDirectory=\${NIFI_REGISTRY_HOME}" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "ExecStart=\${NIFI_REGISTRY_HOME}/bin/nifi-registry.sh start" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "ExecStop=\${NIFI_REGISTRY_HOME}/bin/nifi-registry.sh stop" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "ExecRestart=\${NIFI_REGISTRY_HOME}/bin/nifi-registry.sh restart" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "Restart=on-abnormal" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "[Install]" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "WantedBy=multi-user.target" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
			echo "" | sudo tee -a /etc/systemd/system/nifi-registry.service > /dev/null
		EOF
		)"
	fi

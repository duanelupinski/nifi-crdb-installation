...
SENSITIVE_KEY=tHiSiSnTaSaFeKeY
...
REGISTRY_LOG_DIR="/var/log/nifi-registry"  # override with --registry-log-dir
...
ni::op::new_cluster(){
  ...
  #   ni::certs::ensure_cluster_tls "${installer}" "${NIFI_USER}" "${KEYSTORE_PASSWD}" "${TRUSTSTORE_PASSWD}" "${peers_for_certs[@]}"
  # fi
  ...
  # for n in "${NIFI_NODES[@]}"; do
  #   remote="${installer}@$(ni::resolve_host "${n}")"
  #   ni::nifi::configure "${remote}" "${secure}" "${ZK_CONNECT}" "${NIFI_USER}" "${SENSITIVE_KEY}" "${KEYSTORE_PASSWD}" "${TRUSTSTORE_PASSWD}"
  # done
  # if [ -n "${registry_host}" ]; then
  #   remote="${installer}@${registry_host}"
  #   ni::registry::configure "${remote}" "${secure}" "${registry_host}" "${NIFI_USER}" "${KEYSTORE_PASSWD}" "${TRUSTSTORE_PASSWD}"
  # fi
  ...
  # if [ -n "${registry_host}" ]; then
  #   remote="${installer}@$(ni::resolve_host "${registry_host}")"
  #   ni::registry::configure_logging "${remote}" "${NIFI_USER}" "${REGISTRY_LOG_DIR}" "${LOG_MAX_SIZE}" "${LOG_MAX_HISTORY}"
  # fi

...
for nic in ${NICS}; do
  ...
  if [[ "${TYPE}" = "bridged" ]]; then
    ADAPTER=$("${VBOXMANAGE}" list bridgedifs | head -n 1 | awk '{split($0,a,":"); print a[2]}' | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')
    "${VBOXMANAGE}" modifyvm ${VMNAME} --nic${ID} ${TYPE} --bridge-adapter${ID} "${ADAPTER}"
  fi
